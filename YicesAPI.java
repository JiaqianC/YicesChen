...
/**
 * Calls yices 2 (It is the copy of the yices1 call)
 *
 * 
 */
public class YicesAPI extends SMTEngine {
    ...
    public YicesAPI(final Arguments arguments) {
        super(arguments);
        this.args = arguments;
    }
    ...
    public Pair<YNM, Map<String, String>> solve(final String smtString, final SMTLogic logic, final Abortion aborter)
        ...
        try {
            ...
            YicesAPI.LOG.log(Level.FINER, "SMT    to {0}\n", input.getCanonicalPath());

            YicesAPI.LOG.log(Level.FINER, "Invoking {0}\nyices2 -e");
            ...
            while (it.hasNext()) {
                ...
                YicesAPI.LOG.log(Level.FINEST, "{0}\n", line);
                //System.err.println("yices2-out: " + line);
                if (line.startsWith("unsat")) {
                    YicesAPI.LOG.log(Level.FINE, "YICES 2 says: UNSAT\n");
                    resType = YNM.NO;
                }
                if (line.startsWith("sat")) {
                    YicesAPI.LOG.log(Level.FINE, "YICES 2 says: SAT\n");
                    resType = YNM.YES;
                }
                if (line.startsWith("unknown")) {
                    YicesAPI.LOG.log(Level.FINE, "YICES 2 says: UNKNOWN\n");
                    resType = YNM.MAYBE;
                }
                if (line.startsWith("(")) {
                    if (line.length() < 4) {
            ...
            if (YicesAPI.LOG.isLoggable(Level.FINE)) {
                YicesAPI.LOG.fine("SMT solving with Yices 2 took " + (nanos2 - nanos1) / 1000000L + " ms.");
            }
            input.delete();
            aborter.checkAbortion();
