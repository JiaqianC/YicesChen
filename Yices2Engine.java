...
/**
 * Calls yices 2 with file
 *
 * 
 */
public class Yices2Engine extends SMTEngine {
    ...
    public Yices2Engine(final Arguments arguments) {
        super(arguments);
        this.args = arguments;
    }
    ...
    public Pair<YNM, Map<String, String>> solve(final String smtString, final SMTLogic logic, final Abortion aborter)
        ...
        try {
            ...
            Yices2Engine.LOG.log(Level.FINER, "SMT    to {0}\n", input.getCanonicalPath());

            Yices2Engine.LOG.log(Level.FINER, "Invoking {0}\nyices2 -e");
            ...
            while (it.hasNext()) {
                ...
                Yices2Engine.LOG.log(Level.FINEST, "{0}\n", line);
                //System.err.println("yices2-out: " + line);
                if (line.startsWith("unsat")) {
                    Yices2Engine.LOG.log(Level.FINE, "YICES 2 says: UNSAT\n");
                    resType = YNM.NO;
                }
                if (line.startsWith("sat")) {
                    Yices2Engine.LOG.log(Level.FINE, "YICES 2 says: SAT\n");
                    resType = YNM.YES;
                }
                if (line.startsWith("unknown")) {
                    Yices2Engine.LOG.log(Level.FINE, "YICES 2 says: UNKNOWN\n");
                    resType = YNM.MAYBE;
                }
                if (line.startsWith("(")) {
                    if (line.length() < 4) {
            ...
            if (Yices2Engine.LOG.isLoggable(Level.FINE)) {
                Yices2Engine.LOG.fine("SMT solving with Yices 2 took " + (nanos2 - nanos1) / 1000000L + " ms.");
            }
            input.delete();
            aborter.checkAbortion();
