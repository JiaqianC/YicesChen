 ...
 * 
 */
public class Yices2Com extends SMTEngine {
    ...
    public Yices2Com(final Arguments arguments) {
        super(arguments);
        this.args = arguments;
    }
    ...
    public Pair<YNM, Map<String, String>> solve(final String smtString, final SMTLogic logic, final Abortion aborter)
        ...
        try {
            ...
            Yices2Com.LOG.log(Level.FINER, "SMT    to {0}\n", input.getCanonicalPath());

            Yices2Com.LOG.log(Level.FINER, "Invoking {0}\nyices -e");
            ...
            while (it.hasNext()) {
                ...
                Yices2Com.LOG.log(Level.FINEST, "{0}\n", line);
                //System.err.println("yices-out: " + line);
                if (line.startsWith("unsat")) {
                    Yices2Com.LOG.log(Level.FINE, "YICES says: UNSAT\n");
                    resType = YNM.NO;
                }
                if (line.startsWith("sat")) {
                    Yices2Com.LOG.log(Level.FINE, "YICES says: SAT\n");
                    resType = YNM.YES;
                }
                if (line.startsWith("unknown")) {
                    Yices2Com.LOG.log(Level.FINE, "YICES says: UNKNOWN\n");
                    resType = YNM.MAYBE;
                }
                if (line.startsWith("(")) {
                    if (line.length() < 4) {
            ...
            if (Yices2Com.LOG.isLoggable(Level.FINE)) {
                Yices2Com.LOG.fine("SMT solving with Yices took " + (nanos2 - nanos1) / 1000000L + " ms.");
            }
            input.delete();
            aborter.checkAbortion();
